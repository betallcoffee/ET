...
Buffer::Buffer(int capacity)
    ...
    size_(0)
{
    alloc_ = Alloc::sharedInstance();
    first_ = (BufferChunk *)alloc_->allocate(sizeof(BufferChunk));
    if (first_ != NULL) {
        first_->next_ = NULL;
...
int Buffer::read(char *to, int size)
{
    int pos = 0;
    BufferChunk *tmp = first_;
    while (tmp != NULL) {
...
int Buffer::write(const char *from, int size)
{
    int pos = 0;
...
int Buffer::write(Buffer *from)
{
    int res = 0;
    BufferChunk *tmp;
    while (tmp = from->pop(), tmp) {
...
int Buffer::readToFD(int fd, int size)
{
    return 0;
}

int Buffer::writeFromFD(int fd)
{
    return 0;
}

int Buffer::swap(Buffer *b)
{
    int res = 0;
    BufferChunk *tmp;
    while (tmp = b->pop(), tmp) {
...
void Buffer::push(BufferChunk *b)
{
...
int Buffer::expend(int size)
{
    size = size > kExpendSize ? size : kExpendSize;
    if (pos_ == NULL) {
        pos_ = (BufferChunk *)alloc_->allocate(sizeof(BufferChunk));
        if (pos_ == NULL) {
            pos_ = last_;
            return -1;
        } else {
